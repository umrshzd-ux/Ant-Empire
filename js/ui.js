...
// DOM references (must be called after DOM is ready)
...
function updateBossTimer() {
  if (state.bossActive) {
    ...
    var sec = Math.ceil(state.bossTimer);
    elBossTimer.textContent = "Boss " + sec + "s";
    elBossTimer.parentElement.style.borderColor = sec < 10 ? "#cc0000" : "";
  }
}
...
function updateBuildButtons() {
  var bfs = document.getElementById("build-food-storage"), bn = document.getElementById("build-nursery"), bs = document.getElementById("build-soldier"), br = document.getElementById("build-research"), bsc = document.getElementById("build-scout");
...
// Main menu and game buttons setup
...
function setupButtons() {
  ...
  var closeAll = function() {
    buildPanel.classList.remove("open"); evoPanel.classList.remove("open"); upgradePanel.classList.remove("open");
    shopPanel.classList.remove("open"); achPanel.classList.remove("open"); ppPanel.classList.remove("open");
    ascPanel.classList.remove("open");
  };
  if (btnBuild) btnBuild.onclick = function() { AudioManager.sfx.buttonClick(); if (buildPanel.classList.contains("open")) closeAll(); else { closeAll(); buildPanel.classList.add("open"); updateBuildButtonLabels(); } };
  if (btnEvo) btnEvo.onclick = function() { AudioManager.sfx.buttonClick(); if (evoPanel.classList.contains("open")) closeAll(); else { closeAll(); evoPanel.classList.add("open"); refreshEvolutionUI(); } };
  if (btnUpgrades) btnUpgrades.onclick = function() { AudioManager.sfx.buttonClick(); if (upgradePanel.classList.contains("open")) closeAll(); else { closeAll(); upgradePanel.classList.add("open"); refreshUpgradeUI(); } };
  if (btnShop) btnShop.onclick = function() { AudioManager.sfx.buttonClick(); if (shopPanel.classList.contains("open")) closeAll(); else { closeAll(); shopPanel.classList.add("open"); } };
  if (btnAch) btnAch.onclick = function() { AudioManager.sfx.buttonClick(); if (achPanel.classList.contains("open")) closeAll(); else { closeAll(); achPanel.classList.add("open"); refreshAchievementsUI(); } };
  if (btnPP) btnPP.onclick = function() { AudioManager.sfx.buttonClick(); if (ppPanel.classList.contains("open")) closeAll(); else { closeAll(); ppPanel.classList.add("open"); refreshPrestigeShopUI(); } };
  if (btnAscShop) btnAscShop.onclick = function() { AudioManager.sfx.buttonClick(); if (ascPanel.classList.contains("open")) closeAll(); else { closeAll(); ascPanel.classList.add("open"); refreshAscensionShopUI(); } };
  if (btnDaily) btnDaily.onclick = function() { AudioManager.sfx.buttonClick(); var dp = document.getElementById('daily-panel'); dp.style.display = dp.style.display === 'flex' ? 'none' : 'flex'; refreshDailyUI(); };
  ...
  updateBuildButtons();
  // Show research-dependent buttons if applicable
