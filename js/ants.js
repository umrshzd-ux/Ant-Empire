...
function updateWorker(w, dt) {
  ...
  if (w.state === "AT_NEST") {
    addFood(fpt, NP.clone(), "worker");
    addStockpileCrumb(); storagePilesDirty = true;
    qgLight.intensity = 3; qgSphere.material.emissiveIntensity = 1.5; cLP = 1;
    w.carrying = false; w.dropAnimTimer = 0.4; w.waitTimer = 0.4;
    w.state = "TO_FOOD"; setPathTarget(w, "FOOD");
    return;
  }
  ...
  if (w.carrying) {
    ...
    if (!w.foodIcon) {
      ...
      ic.position.set(0, 0.55, 0); w.mesh.add(ic); w.foodIcon = ic;
    }
...
function updateSoldier(s, dt) { /* unchanged – same as original */ }
...
function layEgg() { /* unchanged – same as original */ }
function spawnHatchSparkles(pos) { /* unchanged – same as original */ }
function hatchEgg(egg, i) { /* unchanged – same as original */ }
