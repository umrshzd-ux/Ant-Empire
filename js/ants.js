...
function claimStationSlot(st, w) {
  ...
  return Math.floor(Math.random() * SPS); // fallback if full
}
...
function createWorker(golden, rareType, forceRender) {
  ...
  var mesh = null, ws = getWorkerVisualScale();
  ...
  var w = {
    ...
    carryingEgg: false,
    avoidTimer: 0,
    isGolden: golden || false,
    isRare: !!rareType,
    rareType: rareType,
    foodBonus: 0,
    _speedMult: speedMult,
    birthTimer: undefined,
    birthDuration: undefined,
    dropAnimTimer: undefined
  };
...
function updateWorker(w, dt) {
  ...
  if (isEnemyNearby(w, BAL.workerFleeRange)) {
    ...
    if (nearestPos) {
      var dx = w.mesh.position.x - nearestPos.x, dz = w.mesh.position.z - nearestPos.z;
  ...
  // Avoid soldiers (only if not returning home near entrance)
  ...
  if (w.birthTimer !== undefined && w.birthTimer > 0) {
    w.birthTimer -= dt;
    var t = 1 - Math.max(0, w.birthTimer / w.birthDuration);
    var e = t*t*(3-2*t);
    w.mesh.scale.setScalar(w.targetScale*(0.05 + 0.95*e));
    if (w.birthTimer <= 0) {
      w.mesh.scale.setScalar(w.targetScale);
      w.birthTimer = undefined;
    }
  }

  // Drop animation for food icon
  ...
  var raw = w.path[w.pathIndex];
  if (!raw) return;
  var isFinal = (w.pathIndex === w.path.length - 1);
  var target;
  if (isFinal) {
    if (w.state === "TO_FOOD") {
  ...
  } else if (w.foodIcon && w.dropAnimTimer === undefined) {
    disposeMesh(w.foodIcon);
    w.mesh.remove(w.foodIcon);
    w.foodIcon = null;
  }
...
function createHealthBar(parent, w, h, yOff) {
  ...
  sp.position.set(0, yOff, 0); sp.scale.set(w/50, h/50, 1);
  parent.add(sp);
...
function updateHealthBar(bar, ratio) {
  ...
  ctx.fillStyle = "#333"; ctx.fillRect(0, 0, bar.canvas.width, bar.canvas.height);
...
function soldierDied(soldier) {
  ...
  if (idx >= 0) {
    if (soldier.guardMesh) { disposeMesh(soldier.guardMesh); scene.remove(soldier.guardMesh); var gi = barracksSoldiers.indexOf(soldier.guardMesh); if (gi >= 0) barracksSoldiers.splice(gi, 1); }
    disposeMesh(soldier.mesh); scene.remove(soldier.mesh);
    soldiers.splice(idx, 1);
  }
  state.soldierCount--; state.deadSoldiers++; state.soldierRespawnTimer = BAL.soldierRespawnTime;
  showToast("💀 Soldier fallen!");
  if (state.bossActive && soldiers.length === 0) { resolveBossFight("defeat"); }
}
function respawnSoldier() {
  state.soldierCount++; state.deadSoldiers--;
...
function updateSoldier(s, dt) {
  ...
  // Push away from entrance to prevent blocking
  if (distToNest < 2.5) {
...
function hatchEgg(egg, i) {
  var hp = egg.mesh.position.clone(); disposeMesh(egg.mesh); scene.remove(egg.mesh);
  eggMs.splice(i, 1);
  state.eggs = Math.max(0, state.eggs - 1);
  state.workerCount++; state.totalHatched++; state.lifetimeStats.totalHatched++;
  updateEggLayTime();
  var rt = null;
  if (Math.random() < 0.08) { rt = RARE_TYPES[Math.floor(Math.random()*RARE_TYPES.length)]; state.rareAntCount++; updateDailyProgress('rare1', 1); }
  var nw = createWorker(false, rt);
  if (nw) {
    nw.mesh.position.copy(hp); nw.mesh.scale.setScalar(0.05);
    nw.birthTimer = 0.5; nw.birthDuration = 0.5; nw.waitTimer = 0.5;
  ...
  updateNurseryClusters(); updateDailyProgress('hatch5', 1);
  if (rt) showToast(rt.emoji + " Rare " + rt.name + " hatched!"); checkAchievements(); pTH();
}
...
(function(AM) {
  ...
  AM.init = function() {
    ...
    if (ctx) {
      ambientGain = ctx.createGain(); ambientGain.gain.value = 0.06; ambientGain.connect(ctx.destination);
      if (ambientOn) AM.startAmbient();
      if (musicOn) AM.startMusic();
    }
  };
  AM.resume = function() {
    if (ctx && ctx.state === 'suspended') ctx.resume().then(function() { if (musicOn && musicNodes.length === 0) AM.startMusic(); });
  };
  AM.playTone = function(freq, dur, vol, type, rampDown) {
    ...
    o.connect(g); g.connect(ctx.destination); o.start(); o.stop(ctx.currentTime + (dur || 0.1) + (rampDown || 0.03));
  };
  ...
  AM.startAmbient = function() {
    ...
    windOsc.start(); ambientNode.start(); ambientNode.windOsc = windOsc;
  };
  AM.stopAmbient = function() {
    if (ambientNode) { try { ambientNode.stop(); } catch(e) {} if (ambientNode.windOsc) { try { ambientNode.windOsc.stop(); } catch(e) {} } ambientNode = null; }
  };
  AM.startMusic = function() {
    if (!ctx || !musicOn) return; AM.stopMusic();
    var now = ctx.currentTime, baseFreq = 130.81, chord = [1,5/4,3/2,2], masterGain = ctx.createGain(); masterGain.gain.value = 0.12; masterGain.connect(ctx.destination);
    chord.forEach(function(ratio,i){ var osc = ctx.createOscillator(); osc.type='sine'; osc.frequency.setValueAtTime(baseFreq*ratio,now); var vol = ctx.createGain(); vol.gain.setValueAtTime(0.025,now); vol.gain.exponentialRampToValueAtTime(0.015,now+2); osc.connect(vol); vol.connect(masterGain); osc.start(now+i*0.1); musicNodes.push({osc:osc,gain:vol}); });
  ...
  AM.stopMusic = function() {
    ...
    if (masterGainEntry && masterGainEntry.masterGain) {
      ...
      setTimeout(function(){ musicNodes.forEach(function(node){ try { if (node.osc) node.osc.disconnect(); } catch(e) {} try { if (node.gain) node.gain.disconnect(); } catch(e) {} try { if (node.masterGain) node.masterGain.disconnect(); } catch(e) {} }); musicNodes=[]; },100);
    } else {
      musicNodes.forEach(function(node){ try { if (node.osc) node.osc.stop(); } catch(e) {} try { if (node.osc) node.osc.disconnect(); } catch(e) {} try { if (node.gain) node.gain.disconnect(); } catch(e) {} });
      musicNodes=[];
    }
  };
  AM.setMusic = function(on) { musicOn=on; localStorage.setItem('antEmpire_music',on?'1':'0'); if(on){ if(ctx&&ctx.state==='running') AM.startMusic(); else if(ctx) AM.resume(); } else AM.stopMusic(); };
  AM.setSfx = function(on) { sfxOn=on; localStorage.setItem('antEmpire_sfx',on?'1':'0'); };
  AM.setAmbient = function(on) { ambientOn=on; localStorage.setItem('antEmpire_ambient',on?'1':'0'); if(on) AM.startAmbient(); else AM.stopAmbient(); };
})(AudioManager);
...
function triggerHaptic(duration, intensity) {
  ...
  if (window.navigator && window.navigator.vibrate) window.navigator.vibrate(vibDuration);
}
